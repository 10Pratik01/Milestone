...
async function safeCreateMany(delegate, data) {
...
async function main() {
    ...
    if (projects.length) {
        ...
        await safeCreateMany(prisma.project, projects);
    }
    ...
    if (teamsWithoutOwners.length) {
        ...
        await safeCreateMany(prisma.team, teamsWithoutOwners);
    }
    // 3) Users (users may reference teamId so teams must exist)
    const users = readJson("user.json");
    if (users.length) {
        console.log("Seeding users...");
        await safeCreateMany(prisma.user, users);
    }
    ...
    if (rawTeams.length) {
        ...
        for (const original of rawTeams) {
            ...
            try {
                ...
                const team = await prisma.team.findFirst({ where: { teamName } });
                if (!team) {
                    console.warn(`Team not found to update owners: ${teamName}`);
                    continue;
                }
                const updates = {};
                if (original.productOwnerUserId)
                    updates.productOwnerUserId = original.productOwnerUserId;
                if (original.productManagerUserId)
                    updates.productManagerUserId = original.productManagerUserId;
                if (Object.keys(updates).length === 0)
                    continue;
                await prisma.team.update({ where: { id: team.id }, data: updates });
    ...
    if (projectTeams.length) {
        ...
        await safeCreateMany(prisma.projectTeam, projectTeams);
    }
    // 6) tasks
    const tasks = readJson("task.json");
    if (tasks.length) {
        console.log("Seeding tasks...");
        await safeCreateMany(prisma.task, tasks);
    }
    // 7) attachments
    const attachments = readJson("attachment.json");
    if (attachments.length) {
        console.log("Seeding attachments...");
        await safeCreateMany(prisma.attachment, attachments);
    }
    // 8) comments
    const comments = readJson("comment.json");
    if (comments.length) {
        console.log("Seeding comments...");
        await safeCreateMany(prisma.comment, comments);
    }
    // 9) taskAssignment
    const taskAssignments = readJson("taskAssignment.json");
    if (taskAssignments.length) {
        console.log("Seeding taskAssignment...");
        await safeCreateMany(prisma.taskAssignment, taskAssignments);
    }
